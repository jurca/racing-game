...
export function makeScene(data: SceneData): Scene {
  ...
  const track = new Track(new Vector3(0, 0, 0), segments, RENDER_DISTANCE)
  scene.addSubObject(track)

  return scene
}

export const defaultCamera = new Pseudo3DCamera(
  new Vector3(0, BASE_CAMERA_HEIGHT, -8_000),
  VIEWPORT_WIDTH,
  VIEWPORT_HEIGHT,
  60 / 180 * Math.PI,
)
